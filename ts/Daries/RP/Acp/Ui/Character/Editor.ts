...
export class AcpUiCharacterEditor {
  ...
  async #clipboardAction(actionData: ClipboardActionData): Promise<void> {
    ...
    if (actionData.data.actionName === "dev.daries.rp.character.enable") {
      ...
      unmarkClipboard("dev.daries.rp.character", characterIds);
    }
  }
  ...
  #initHandler(): void {
    addHandler("dev.daries.rp.acp.character", "refresh", (data: RefreshCharactersData) => {
      this.#refreshCharacters(data);
    });

    addHandler("com.woltlab.wcf.clipboard", "dev.daries.rp.character", (data: ClipboardActionData) => {
...
interface ClipboardActionData {
  data: {
    actionName: "dev.daries.rp.character.delete" | "dev.daries.rp.character.enable";
    parameters: {
