<script data-relocate="true">
	require(['Language', 'WoltLabSuite/Core/Ui/ItemList/Filter',
		'Daries/RP/Form/Builder/Field/Character/MultipleSelection'], (Language,
	UiItemListFilter, { setup: setupCharacterMultipleSelection }) => {
		Language.addObject({
		...
		setupCharacterMultipleSelection('{$field->getPrefixedId()}_list')
	});
</script>
		...
		<li{if $__fieldNestedOption[depth] > 0} style="padding-left: {$__fieldNestedOption[depth]*20}px" {/if}>
			<label>
				<input {*
					...
					*}{if !$field->getFieldClasses()|empty}
						class="{implode from=$field->getFieldClasses() item='class' glue=' '}{$class}{/implode}"
						{/if}{*
					...
					*}{foreach from=$field->getFieldAttributes() key='attributeName' item='attributeValue'}
						{$attributeName}="{$attributeValue}" {/foreach} {*
											*} 
							{if $__fieldNestedOption[userID]} data-user-id="{$__fieldNestedOption[userID]}" 
							{/if}{*
										*}>
